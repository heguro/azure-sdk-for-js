...
const { pages, tables, styles, keyValuePairs, content, documents } = await poller.pollUntilDone();
```
...
```ts snippet:ReadmeSampleAnalyzeDocumentPrebuiltLayout
import { createReadStream } from "node:fs";
import { DocumentAnalysisClient, AzureKeyCredential } from "@azure/ai-form-recognizer";
...
```ts snippet:ReadmeSampleAnalyzeDocumentPrebuiltRead
import {
  DocumentSpan,
  DocumentAnalysisClient,
  AzureKeyCredential,
